...
import (
	...
	"github.com/c0d3-5t3w/GoNetPad/internal/app/capture"
	"github.com/c0d3-5t3w/GoNetPad/internal/app/config"
	"github.com/c0d3-5t3w/GoNetPad/internal/app/locale"
	"github.com/c0d3-5t3w/GoNetPad/internal/app/logger"
	"github.com/c0d3-5t3w/GoNetPad/internal/app/themes"
...
func main() {
	...
	a.Settings().SetTheme(themes.OriginalTheme)
	...
	formatButton := widget.NewButton("Format Code", func() {
		...
		formatted, err := formatCode(textArea.Text)
		if err != nil {
	...
	menu := fyne.NewMainMenu(
		...
		fyne.NewMenu("Themes",
			fyne.NewMenuItem("Original Theme", func() {
				a.Settings().SetTheme(themes.OriginalTheme)
			}),
			fyne.NewMenuItem("Inverted Theme", func() {
				a.Settings().SetTheme(themes.InvertedTheme)
			}),
			fyne.NewMenuItem("Dark Red Theme", func() {
				a.Settings().SetTheme(themes.DarkRedTheme)
			}),
		),
	)
...
func formatCode(content string) (string, error) {
